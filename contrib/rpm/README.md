...
The RPM spec file provided here is for Pexa-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://pexacore.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Pexa git repository](https://github.com/pexacoin/core) but are not part of
...
Currently only `Source10` is used, for the example `pexa.conf` file.
...
in the pexa git repository. At present only an SVG version of the Pexa
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the pexa
...
needed when 0.12.1 is released, a proper fix is already in the Pexa git
master branch.
...
idea to build Pexa Core against a static version of that library compiled
...
The `pexa.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `pexa-server` package. At this point, most RPM based
...
The `pexa-server` package should have SELinux support. How to properly do
...
Pexa-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/pexa/). He has been
packaging Pexa for Fedora at least since 2012.
...
4. I renamed the `pexa` package that contains the Qt GUI to `pexa-core` as
that appears to be how the general population refers to it, in contrast to
`pexa-xt` or `pexa-classic`. I wanted to make sure the general population
