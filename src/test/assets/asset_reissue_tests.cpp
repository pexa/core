...
BOOST_FIXTURE_TEST_SUITE(asset_reissue_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(reissue_cache_test)
    {
        ...
        CNewAsset asset1("PEXAASSET", CAmount(100 * COIN), 8, 1, 0, "");

        // Add an asset to a valid pexa address
        ...
        CReissueAsset reissue1("PEXAASSET", CAmount(1 * COIN), 8, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        BOOST_CHECK_MESSAGE(cache.mapReissuedAssetData.count("PEXAASSET"), "Map Reissued Asset should contain the asset \"PEXAASSET\"");
        BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("PEXAASSET", Params().GlobalBurnAddress())) == CAmount(101 * COIN), "Reissued amount wasn't added to the previous total");

        // Get the new asset data from the cache
        CNewAsset asset2;
        BOOST_CHECK_MESSAGE(cache.GetAssetMetaDataIfExists("PEXAASSET", asset2), "Failed to get the asset2");
        ...
        BOOST_CHECK_MESSAGE(asset2.strName == "PEXAASSET", "Asset2: Asset name is wrong");
        ...
        undoBlockData.emplace_back(std::make_pair("PEXAASSET", CBlockAssetUndo{true, false, "", 0}));
        ...
        CNewAsset asset3;
        BOOST_CHECK_MESSAGE(cache.GetAssetMetaDataIfExists("PEXAASSET", asset3), "Failed to get the asset3");
        ...
        BOOST_CHECK_MESSAGE(asset3.strName == "PEXAASSET", "Asset3: Asset name is wrong");
        ...
        BOOST_CHECK_MESSAGE(cache.mapReissuedAssetData.count("PEXAASSET"), "Map of reissued data was removed, even though changes were made and not databased yet");
        BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("PEXAASSET", Params().GlobalBurnAddress())) == CAmount(1 * COIN), "Assets total wasn't undone when reissuance was");
    }
    ...
    BOOST_AUTO_TEST_CASE(reissue_isvalid_test)
    {
        ...
        CNewAsset asset1("PEXAASSET", CAmount(1 * COIN), 8, 1, 0, "");
        ...
        CReissueAsset reissue3("PEXAASSET", CAmount(1 * COIN), 7, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueAsset reissue4("PEXAASSET", CAmount(1 * COIN), -1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));

        BOOST_CHECK_MESSAGE(reissue4.IsValid(error, cache), "Reissue4 wasn't valid");

        // Create a new asset object with units of 0
        CNewAsset asset2("PEXAASSET2", CAmount(1 * COIN), 0, 1, 0, "");

        // Add new asset to a valid pexa address
        ...
        CReissueAsset reissue5("PEXAASSET2", CAmount(1 * COIN), 1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueAsset reissue6("PEXAASSET2", CAmount(1 * COIN), 1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
