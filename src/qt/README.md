This directory contains the PexaQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Pexa UI.
...
ClientModel is used by the main application `pexagui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a pexa:... link from a browser).
...
* UI elements like PexaAmountField, which inherit from QWidget.
* `pexastrings.cpp`: automatically generated
* `pexaunits.(h/cpp)`: PEXA / mPEXA / etc handling
* `callback.h`
...
* don't change `local/pexa_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "pexa-qt" under "Run", which is where you can also add command line arguments)
