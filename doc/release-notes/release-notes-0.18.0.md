Pexa Core version 0.18.0 is now available from:

  <https://pexacore.org/bin/pexa-core-0.18.0/>
...
`/Applications/Pexa-Qt` (on Mac) or `pexad`/`pexa-qt` (on
Linux).
...
recommended to use Pexa Core on unsupported systems.
...
10.10. Additionally, Pexa Core does not yet change appearance when
...
`contrib/init/pexad.service`, it has been changed to use
`/var/lib/pexad` as the data directory instead of
`~pexa/.pexa`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/pexad` will
exist has enough space (check using `df -h /var/lib/pexad`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/pexa/pexa/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://pexacore.org/en/contact/) the Pexa Core
...
The systemd init file (`contrib/init/pexad.service`) has been changed
to use `/var/lib/pexad` as the data directory instead of
`~pexa/.pexa`. This change makes Pexa Core more consistent with
...
`datadir` option in `/etc/pexa/pexa.conf` will have no effect.
...
- A new short [document](https://github.com/pexa/pexa/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/pexa/pexa/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/pexa/pexa/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/pexa/pexa/blob/master/doc/pexa-conf.md)
  about the `pexa.conf` file describes how to use it to configure
  Pexa Core.

- A new document introduces Pexa Core's BIP174 [Partially-Signed
  Pexa Transactions
  (PSBT)](https://github.com/pexa/pexa/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/pexa/pexa/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Pexa-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Pexa Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/pexa/pexa/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/pexa/pexa/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Pexa Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Pexa Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/pexa/pexa/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/pexa/pexa/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart pexad with the `-deprecatedrpc=generate`
...
- A new `pexa-wallet` tool is now distributed alongside Pexa
...
This section describes planned changes to Pexa Core that may affect
other Pexa software and services.

- Since version 0.16.0, Pexa Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Pexa Core 0.20 (expected about
  a year after 0.18), Pexa Core will default to native segwit
  ...
  to bech32 addresses, and if the Pexa Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Pexa Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any Pexa Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Pexa Core
...
- A sub-project of Pexa Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Pexa Core.  See their [project
  page](https://github.com/pexa-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Pexa Core's own implementation, although entropy
  gathered by Pexa Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Pexa Core a
...
- On macOS, Pexa Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `pexa_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libpexa_server.a` (jnewbery)
...
- #13926 [Tools] pexa-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like pexad (MarcoFalke)
...
- #14521 qt, docs: Fix `pexa-qt -version` output formatting (hebasto)
...
- #11625 Add PexaApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libpexa_server` (laanwj)
...
- #15154 Configure: pexa-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/pexa-wallet (Sjors)
...
- #11526 Visual Studio build configuration for Pexa Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libpexa_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in pexa-util-test.py (isghe)
...
- #12775 Integration of property based testing into Pexa Core (Christewart)
...
- #12246 Bugfix: Only run pexa-tx tests when pexa-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_pexa` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_pexa` (MarcoFalke)
...
- #14186 pexa-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_pexa.conf` message on startup if conf file exists (leishman)
...
- #12255 Update pexa.service to conform to init.md (dongcarl)
...
- #14497 Add doc/pexa-conf.md (hebasto)
...
- #15354 Add missing `pexa-wallet` tool manpages (MarcoFalke)
...
- #15765 correct pexaconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support pexa folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/pexa/).
