...
def main():
    ...
    tmpdir = "%s/pexa_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/pexacoin/core/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/pexacoin/core/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_pexad):
        print("No functional tests to run. Wallet, utils, and pexad must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if pexad is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "pexad"]) is not None:
    ...
    #Set env vars
    if "PEXAD" not in os.environ:
        os.environ["PEXAD"] = build_dir + '/src/pexad' + exeext
        os.environ["PEXACLI"] = build_dir + '/src/pexa-cli' + exeext
