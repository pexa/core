...
from test_framework.address import ADDRESS_XCRT1_P2WSH_OP_TRUE
...
class P2PPermissionsTests(PexaTestFramework):
    ...
    def check_tx_relay(self):
        block_op_true = self.nodes[0].getblock(self.nodes[0].generatetoaddress(100, ADDRESS_XCRT1_P2WSH_OP_TRUE)[0])
        self.sync_all()
    ...
    def replaceinconfig(self, nodeid, old, new):
        with open(self.nodes[nodeid].pexaconf, encoding="utf8") as f:
            newText = f.read().replace(old, new)
        with open(self.nodes[nodeid].pexaconf, 'w', encoding="utf8") as f:
            f.write(newText)
