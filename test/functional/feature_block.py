...
class FullBlockTest(PexaTestFramework):
    ...
    def run_test(self):
        ...
        # Extend the b26 chain to make sure pexad isn't accepting b26
        ...
        # Extend the b28 chain to make sure pexad isn't accepting b28
        ...
        # pexad doesn't disconnect us for sending a bloated block, but if we subsequently
        ...
        #    Low-S and High-S values (whereas the pexa code has custom code which does so);
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, script=CScript([OP_TRUE]), *, version=1):
        ...
        coinbase.rehash()
        if spend is None:
            block = create_block(self.nodes[0], base_block_hash, coinbase, block_time, version=version, prevheight=height-1)
        else:
